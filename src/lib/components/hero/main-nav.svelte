...
{#if Array.isArray(translations) && translations.length === 1}
	{#if heroMenuVisible}
		<a
			...
			class="bg-secondary cursor-pointer p-16 size-64 rounded-full opacity-50 text-common-white hover:bg-red hover:opacity-100 focus:opacity-100 focus:bg-red transition z-50"
			type="button"
			href="#top"
			transition:fade
		>
